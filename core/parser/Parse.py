...
def parser_main() -> dict:
    ...
    try:
        ...
        # На случай, если модуль запускают не как пакет
        from core import config as core_config  # type: ignore

    # Загружаем настройки так же, как это делает интерфейс
    settings = core_config.load_settings()
    gns = settings.get("gns3_server", {})

    login = gns.get("login") or "admin"
    password = gns.get("password") or "*"
    ip = gns.get("ip") or "192.168.56.10"
    port = gns.get("port") or "80"
    project_name = gns.get("project_name") or "untitled"

    base_url = f"http://{ip}:{port}"
    print(f"[parser] Подключение к GNS3 серверу {base_url}, проект '{project_name}'")
    ...
    if not getattr(lab, "opened", False):
        print("[parser] Открываем проект...")
        lab.open()

    # Запускаем все узлы перед сбором конфигов
    print("[parser] Запускаем все узлы проекта...")
    for node in lab.nodes:
        try:
            node.start()
            time.sleep(0.5)
        except Exception as e:
            print(f"[parser][WARN] Не удалось запустить узел {node.name}: {e}")
    ...
    print("[parser] Собираем конфиги/информацию по узлам...")
    ...
    # Останавливаем узлы и закрываем проект
    print("[parser] Останавливаем все узлы...")
    for node in lab.nodes:
        try:
            node.stop()
            time.sleep(0.5)
        except Exception as e:
            print(f"[parser][WARN] Не удалось остановить узел {node.name}: {e}")

    lab.close()
    print("[parser] Проект закрыт, данные собраны.")

    # То, что дальше пойдёт в cli_app и сохранится уже на рабочий стол
    return {
...
if __name__ == "__main__":
    # Режим "запустил в IDE по Run — дальше скрипт сам всё спросит".
    import argparse
    ...
    # дефолты под твой стенд можно менять
